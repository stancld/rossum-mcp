...
# Global list to track generated files
_generated_files: list[str] = []


def get_generated_files() -> list[str]:
    """Get list of files generated by write_file tool."""
    return _generated_files.copy()
...
def write_file(file_path: str, content: str, overwrite: bool = True) -> str:
    ...
    try:
        ...
        # If user provided absolute path, just use filename
        filename = input_path.name if input_path.is_absolute() else str(input_path)
        ...
        if file_path_str not in _generated_files:
            _generated_files.append(file_path_str)

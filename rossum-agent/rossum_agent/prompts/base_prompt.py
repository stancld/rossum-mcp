...
2. **THEN** follow the skill's sandbox-based workflow (deploy_copy_org → sandbox changes → deploy_diff → deploy_to_org)

The deployment skill provides:
- Safe workflow using deployment tools (`deploy_pull`, `deploy_diff`, `deploy_copy_org`, `deploy_to_org`, etc.)

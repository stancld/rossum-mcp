...
class RossumAgent:
    ...
    async def _process_stream_events(
        ...
        while True:
            try:
                item = await asyncio.wait_for(asyncio.to_thread(event_queue.get), timeout=1.5)
            except TimeoutError:
            ...
            if delta:
                if delta.kind == "thinking":
                    ...
                    state.text_buffer.append(delta.content)
                    step_type = (
                        StepType.INTERMEDIATE if state.pending_tools or state.tool_calls else StepType.FINAL_ANSWER
                    )
                    if step := state.flush_buffer(step_num, step_type):
                        yield step

            if (state.pending_tools or state.tool_calls) and (
                step := state.flush_buffer(step_num, StepType.INTERMEDIATE)
            ):
                yield step

...
def _db_safe(
    ...
    """Log `exceptions` raised by the wrapped DB method and return `default` instead.

    Centralizes the try/log/return-sentinel pattern used across PostgresStorage methods.
    """
    ...
    def decorator(func: Callable[P, T]) -> Callable[P, T]:
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
            try:
                ...
                logger.error(f"Database error in {name}: {e}", exc_info=True)
...
class PostgresStorage:
    ...
    @_db_safe(default=False, exceptions=(SQLAlchemyError, OSError))
    ...
    @_db_safe(default=0, exceptions=(SQLAlchemyError, OSError))
